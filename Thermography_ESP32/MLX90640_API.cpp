...
int MLX90640_SetRefreshRate(uint8_t slaveAddr, uint8_t refreshRate)
{
  ...
  //value =1600;//1600; Setting the flowing rate arbitrarily high
  error = MLX90640_I2CRead(slaveAddr, 0x800D, 1, &controlRegister1);
  if (error == 0)
  {
...
void MLX90640_GetImage(uint16_t *frameData, const paramsMLX90640 *params, float *result)
{
  ...
  for ( int pixelNumber = 0; pixelNumber < 768; pixelNumber++)
  {
    ...
    */
    if (true ) //||pattern == frameData[833] ) //why ??
    {
      irData = frameData[pixelNumber];
      if (irData > 32767)
      {
      ...
      irData = irData - params->offset[pixelNumber] * (1 + params->kta[pixelNumber] * (ta - 25)) * (1 + params->kv[pixelNumber] * (vdd - 3.3));
      //Serial.print("irdata corrected : "); Serial.println(irData);
      /*
                  if(mode !=  params->calibrationModeEE)
                  {
                    irData = irData + params->ilChessC[2] * (2 * ilPattern - 1) - params->ilChessC[1] * conversionPattern;
                  }*/
      //result[pixelNumber] = irData;
      irData = irData - params->tgc * irDataCP[subPage];
      //Serial.print("irdata corrected 2: "); Serial.println(irData);
      result[pixelNumber] = irData;

      //alphaCompensated = (params->alpha[pixelNumber] - params->tgc * params->cpAlpha[subPage])*(1 + params->KsTa * (ta - 25));

      //image = irData/alphaCompensated;
      //Serial.print("irdata corrected3 : "); Serial.println(image);
