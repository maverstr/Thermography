...
uint8_t IRTherm::setMax(float maxTemp)
{
  ...
  Serial.print("raw max = "); Serial.println(rawMax);
...
uint8_t IRTherm::setMin(float minTemp)
{
    ...
    Serial.print("raw min = "); Serial.println(rawMin);
...
uint8_t IRTherm::setPWMControl(int16_t data){
  if(writeEEPROM(MLX90614_REGISTER_PWMCTRL, data)){
    return 1;
  }
  return 0;
...
uint8_t IRTherm::setConfigRegister(int16_t data){
  /*int16_t reg;
  I2CReadWord(MLX90614_REGISTER_CONFIG, &reg);
  */
  if(writeEEPROM(MLX90614_REGISTER_CONFIG, data)){
    return 1;
  }
  return 0;
}
